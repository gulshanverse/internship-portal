...
projectRouter.post('/student/tasks/:id/submit', requireRole('STUDENT'), async (req, res, next) => { try { const input = z.object({ content: z.string().max(10000).optional(), repositoryUrl: z.string().url().max(500).optional() }).parse(req.body); return res.status(201).json({ data: await submitTask(req.auth!.user.id, String(req.params.id), input) }); } catch (error) { next(error); } });
