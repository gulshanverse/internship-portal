...
export async function getStudentDashboard(userId: string) {
  ...
  const [applications, projects, notifications, documents] = await prisma.$transaction([
    prisma.application.findMany({ where: { studentId: profile.id }, orderBy: { createdAt: 'desc' }, select: { id: true, publicId: true, studentId: true, internshipId: true, status: true, submittedAt: true, reviewedAt: true, notes: true, createdAt: true, updatedAt: true, internship: { include: { domain: true } }, attempts: { select: { id: true, status: true, percentage: true, passed: true, submittedAt: true }, orderBy: { startedAt: 'desc' }, take: 1 } } }),
