...
import { authorizeResumeDownload, } from './application-service';
...
adminReviewRouter.get('/admin/applications/:id/resume', async (req, res, next) => { try { return res.json({ data: await authorizeResumeDownload(req.auth!.user.id, String(req.params.id)) }); } catch (error) { next(error); } });
